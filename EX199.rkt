...
(define pumpkins2 (make-track "Rhinoceros" "Smashing Pumpkins" "Gish" 1548000 6 date1 200 date1.1))

(define date2 (make-date 1985 2 2 0 0 0))
(define date2.1 (make-date 2018 3 4 0 0 0))
(define U21 (make-track "Where the Streets Have No Name" "U2" "The Joshua Tree" 1487534 0 date2 78 date2.1))

(define date3 (make-date 1979 4 4 0 0 0))
(define date3.1 (make-date 2007 5 5 0 0 0))
(define LedZep1 (make-track "I Can't Quit You Baby" "Led Zeppelin" "Coda" 1347245 3 date3 5 date3.1))
(define LedZep2 (make-track "Wearing and Tearing" "Led Zeppelin" "Coda" 1347245 5 date3 5 date3.1))

(define LTracks (list pumpkins1 pumpkins2 U21 LedZep1 LedZep2))

;select-album-titles/unique
; LTracks > alos

(check-expect (select-album-titles/unique LTracks) (list "Gish" "The Joshua Tree" "Coda"))
(check-expect (select-album-titles/unique '()) '())
(define (select-album-titles/unique lt)
  (cond
    [(empty? lt) '()]
    [else (create-set (select-all-album-titles lt))]))
...
; Ltracks > alos
(check-expect (select-all-album-titles LTracks)
              (list
 "Gish"
 "Gish"
 "The Joshua Tree"
...
;total-time
;Ltracks => hh:mm:ss.00
(check-expect (total-time LTracks) "02:01:18.024")
