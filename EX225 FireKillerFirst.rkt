...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname |EX225 FireKillerFirst|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
;  creeping the scope out to actually make a "pro game". Making 2 levels will teach me as much as 10. So make only 2 with the first being stupid-simple.
...
(define planeState0 [make-planeState 10 0 (/ SCNHGT 2) 1])
(define lake01 [make-groundItem 1600 1750 0 GRASSHGT])
(define lake02 [make-groundItem 5600 5750 0 GRASSHGT])
...
(define burnt0 [make-groundItem 12000 12010 3 GRASSHGT])
(define watered0 [make-groundItem 12020 12030 4 GRASSHGT])
...
(define groundItems0 (list lake01 lake02 mountain01 mountain02 fire0 fire1 fire2 fire3 fire4 burnt0 watered0))
(define ws0 [make-ws 1 1 10000 0 planeState0 5 '() groundItems0])
...
(define groundItems1 (cons (make-groundItem 200 202 1 500) groundItems0))
...
(define (render-visibles logi x)
  (cond
    ...
    [else (overlay/align/offset "left" "bottom"
               (rectangle (- (groundItem-X2 (first logi)) (X1-cropper x (groundItem-X1 (first logi)))) (groundItem-hgt (first logi))  "solid" (get-color (groundItem-type (first logi))))
...
(define PLANEWGT 5)
(define BOMBWGT 5)
(define (lonj-force ps)
   (+ (* -1 G PLANEWGT (sin (rad (planeState-theta ps)))) (* -1 DRAG (planeState-spd ps)) (planeState-thr ps)))  
...
(define (new-spd ps pw)
 (+ (planeState-spd ps) (/ (lonj-force ps) (+ PLANEWGT (* BOMBWGT pw)))))
...
(define (where-bomb-hit-ground wbs)
  (cond
    [(empty? wbs) 0]
    [(< (flyingBomb-Y (first wbs)) 1) (round (flyingBomb-X (first wbs)))]
...
(define (change-thrR ps)
  (make-planeState (planeState-spd ps) (planeState-theta ps) (planeState-hgt ps) 1))
;change-thrF
(define (change-thrF ps)
  (make-planeState (planeState-spd ps) (planeState-theta ps) (planeState-hgt ps) .5))
;change-thrV
...
(define (bomb-hit-mtn? fb gis)
  (cond
    [(empty? gis) false]
    [(and (is-mountain? (first gis)) (hit-mountain? (flyingBomb-X fb) (flyingBomb-Y fb) (first gis))) true]
    [else (bomb-hit-mtn? fb (rest gis))]))
...
(define (end? ws)
  (or (= 0 (ws-tick ws)) (< (planeState-hgt (ws-planeState ws)) 1)))
