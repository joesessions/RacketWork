...
(check-expect (new-tank-speed (make-tank 100 -5) "left") -5)
...
(define (update-key ws ke)
	...
	[(or (string=? ke "left") (string=? ke "right"))
		(make-ws
			...
			(ws-bomb ws)
			(ws-tick ws))]
	[(and (string=? ke " ") (not (missile-active (ws-missile ws))))
		(make-ws
			(ws-tank ws)
			(ws-ufo ws)
...
;**end?
;if missile hits ufo or bomb hits tank, game over.
(define (end? ws)
  (and (< (dfmu (ws-missile ws) (ws-ufo ws)) (/ (image-width UFO) 2)) (< (dfbt (ws-bomb ws) (ws-tank ws)) (/ (image-width TANK) 2)))
     )
